...
import { createInvoiceWithItems, InvoiceItem } from '@/lib/invoices';
...
export default function CreateInvoicePage() {
  ...
  // Fetch products, purchase orders, and delivery orders
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [productsResponse, poResponse, doResponse] = await Promise.all([
          getProducts(),
          getPurchaseOrders(),
          getDeliveryOrders(),
        ]);
        ...
        // Extract delivery orders from response
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await createInvoiceWithItems(data);
      showAlert('Invoice created successfully', 'success');
      router.push('/dashboard/invoices');
    } catch (error) {
      console.error('Error creating invoice:', error);
      showAlert(error instanceof Error ? error.message : 'An error occurred while creating invoice', 'error');
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="mb-6 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              Fill in the details below to create a new invoice with items
            </p>
          </div>
          <button
            type="button"
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-sm text-gray-500">Add products and quantities for this invoice</p>
              </div>
              <button
                type="button"
                onClick={addItem}
