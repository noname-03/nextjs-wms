...
import { getDeliveryOrderWithItems, updateDeliveryOrderWithItems, DeliveryOrderItem } from '@/lib/deliveryOrders';
...
export default function EditDeliveryOrderPage() {
  ...
  const doId = parseInt(params.id as string);
  const { user } = useAuth();
  ...
  // Fetch initial data (DO, products, purchase orders)
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        setIsLoadingData(true);

        // Fetch DO data, products, and purchase orders in parallel
        const [doData, productsResponse, poResponse] = await Promise.all([
          getDeliveryOrderWithItems(doId),
          getProducts(),
          getPurchaseOrders(),
        ]);
        ...
        // Handle DO data
        if (doData) {
          // Parse delivery date to YYYY-MM-DD format for input[type="date"]
          let formattedDeliveryDate = new Date().toISOString().split('T')[0];
          if (doData.deliveryDate) {
            try {
              const dateObj = new Date(doData.deliveryDate);
          ...
          setFormData({
            ...
            purchaseOrderId: doData.purchaseOrderId || 0,
          ...
          setPOSearch(doData.poNumber || '');

          // Populate items with temporary IDs
          if (doData.items && Array.isArray(doData.items)) {
            const populatedItems: DOItemForm[] = doData.items.map((item: any, index: number) => {
              const tempId = `existing-${index}`;
              const itemData: DOItemForm = {
                id: tempId,
                productId: item.productId || 0,
                productName: item.productName || '',
                qtyDelivered: item.qtyDelivered || 1,
        ...
        showAlert('Failed to load delivery order data', 'error');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await updateDeliveryOrderWithItems(doId, data);

      if (response.code === 200) {
        showAlert('Delivery order updated successfully', 'success');
        router.push('/dashboard/delivery-orders');
      } else {
        showAlert(response.message || 'Failed to update delivery order', 'error');
      }
    } catch (error) {
      console.error('Error updating delivery order:', error);
      showAlert('An error occurred while updating delivery order', 'error');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-center justify-end gap-3 pt-4">
            ...
            >
              {isLoading ? 'Updating...' : 'Update Delivery Order'}
            </button>
          </div>
        </form>
      </div>
