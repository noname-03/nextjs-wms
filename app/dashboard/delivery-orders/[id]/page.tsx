...
import { getDeliveryOrderWithItems, DeliveryOrderWithItems } from '@/lib/deliveryOrders';
...
export default function DeliveryOrderDetailPage() {
  ...
  const deliveryOrderId = Number(params.id);
  const printRef = useRef<HTMLDivElement>(null);

  // Delivery Order state
  const [deliveryOrder, setDeliveryOrder] = useState<DeliveryOrderWithItems | null>(null);
  ...
  const fetchDeliveryOrder = useCallback(async () => {
    setIsLoading(true);
    try {
      const data = await getDeliveryOrderWithItems(deliveryOrderId);
      if (data) {
        setDeliveryOrder(data);
      } else {
        showAlert('Error fetching delivery order', 'error');
  ...
  }, [deliveryOrderId, showAlert, router]);

  useEffect(() => {
    if (user && deliveryOrderId) {
      fetchDeliveryOrder();
    }
  }, [user, deliveryOrderId, fetchDeliveryOrder]);
    ...
    return (
      <DashboardLayout>
        <div className="flex justify-center items-center h-64">
          ...
          <span className="ml-3 text-gray-600">Loading delivery order...</span>
        </div>
      </DashboardLayout>
    );
  }
    ...
    return (
      <DashboardLayout>
        <div className="text-center py-12">
          <h3 className="mt-2 text-sm font-medium text-gray-900">Delivery order not found</h3>
          <div className="mt-6">
            <button
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between no-print">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">{deliveryOrder.doNumber}</h1>
        ...
        <div id="printable-area" ref={printRef}>
          ...
          <div className="hidden print:block mb-8">
            <div className="text-center border-b-2 border-gray-800 pb-4 mb-6">
              ...
              <p className="text-sm text-gray-600 mt-2">Document No: {deliveryOrder.doNumber}</p>
            </div>
            <div className="grid grid-cols-2 gap-6 mb-6">
              ...
              <div>
                ...
                <p className="text-sm text-gray-700 font-medium">Purchase Order: {deliveryOrder.poNumber}</p>
                <p className="text-sm text-gray-600">DO Number: {deliveryOrder.doNumber}</p>
              </div>
            </div>
            <div className="grid grid-cols-3 gap-4 mb-6 text-sm">
              <div>
                <span className="font-semibold text-gray-700">Delivery Date:</span>
                <p className="text-gray-900">{formatDate(deliveryOrder.deliveryDate)}</p>
              </div>
              <div>
                <span className="font-semibold text-gray-700">Status:</span>
                <p className="text-gray-900 uppercase">{deliveryOrder.status}</p>
              </div>
              <div>
                <span className="font-semibold text-gray-700">Created:</span>
                <p className="text-gray-900">{formatDate(deliveryOrder.createdAt)}</p>
              </div>
            </div>
          </div>

        {/* Delivery Order Info Card */}
        <div className="bg-white shadow-sm rounded-lg border border-gray-200 overflow-hidden print:shadow-none print:border-0">
          ...
          <div className="px-6 py-5 print:hidden">
            <dl className="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
              <div>
                <dt className="text-sm font-medium text-gray-500">DO Number</dt>
                <dd className="mt-1 text-sm text-gray-900">{deliveryOrder.doNumber}</dd>
              </div>
              <div>
                <dt className="text-sm font-medium text-gray-500">Purchase Order</dt>
                <dd className="mt-1 text-sm text-gray-900">{deliveryOrder.poNumber}</dd>
              </div>
              <div>
                <dt className="text-sm font-medium text-gray-500">Delivery Date</dt>
                <dd className="mt-1 text-sm text-gray-900">{formatDate(deliveryOrder.deliveryDate)}</dd>
              </div>
              <div>
                <dt className="text-sm font-medium text-gray-500">Status</dt>
                <dd className="mt-1">{getStatusBadge(deliveryOrder.status)}</dd>
              </div>
              <div>
                <dt className="text-sm font-medium text-gray-500">Created At</dt>
                <dd className="mt-1 text-sm text-gray-900">{formatDate(deliveryOrder.createdAt)}</dd>
              </div>
              <div>
                <dt className="text-sm font-medium text-gray-500">Last Updated</dt>
                <dd className="mt-1 text-sm text-gray-900">{formatDate(deliveryOrder.updatedAt)}</dd>
              </div>
              {deliveryOrder.description && (
                <div className="sm:col-span-2">
                  <dt className="text-sm font-medium text-gray-500">Description</dt>
                  <dd className="mt-1 text-sm text-gray-900">{deliveryOrder.description}</dd>
                </div>
              )}
            </dl>
          </div>
          ...
          <div className="px-6 py-5 border-b border-gray-200 no-print">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-500">
                  {deliveryOrder.items?.length || 0} item(s) in this delivery order
                </p>
              </div>
            </div>
          </div>
          
          {deliveryOrder.items && deliveryOrder.items.length > 0 ? (
            <>
              {/* Print Table Header */}
              <div className="hidden print:block mb-4">
                <h3 className="text-lg font-semibold text-gray-900">Delivery Items</h3>
              </div>
              ...
              <div className="overflow-x-auto print:block">
                <table className="min-w-full divide-y divide-gray-200 print:border print:border-gray-300">
                    ...
                    {deliveryOrder.items.map((item, index) => (
                      <tr key={item.id} className="hover:bg-gray-50 print:hover:bg-white">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm text-gray-900 print:border print:border-gray-300">
                          {item.qtyDelivered.toLocaleString('id-ID')}
                        </td>
                  ...
                  <tfoot className="bg-gray-50 print:bg-gray-100">
                    <tr>
                      <td colSpan={2} className="px-6 py-4 text-right text-sm font-medium text-gray-900 print:border print:border-gray-300">
                        Total Items:
                      </td>
                      <td className="px-6 py-4 text-right text-lg font-bold text-indigo-600 print:text-gray-900 print:border print:border-gray-300">
                        {deliveryOrder.items.reduce((sum, item) => sum + item.qtyDelivered, 0).toLocaleString('id-ID')}
                      </td>
                ...
                {deliveryOrder.items.map((item, index) => (
                  <div key={item.id} className="px-6 py-4 space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm">
                      <div>
                        <span className="text-gray-500">Quantity Delivered:</span>
            ...
            <div className="px-6 py-12">
              <div className="text-center">
                ...
                <p className="mt-1 text-sm text-gray-500">
                  This delivery order doesn't have any items yet.
                </p>
              </div>
            </div>
          )}
        ...
        <div className="hidden print:block mt-12 pt-8 border-t-2 border-gray-300">
          {deliveryOrder.description && (
            <div className="mb-8">
              ...
              <p className="text-sm text-gray-700">{deliveryOrder.description}</p>
            </div>
          )}
          ...
          <div className="mb-8">
            ...
            <ul className="text-xs text-gray-600 space-y-1 list-disc list-inside">
              ...
              <li>Any discrepancies must be reported within 24 hours</li>
              <li>Signature confirms receipt of items in good condition</li>
              <li>Please keep this DO for your records</li>
            </ul>
          </div>
          ...
          <div className="grid grid-cols-3 gap-8 text-center">
            ...
            <div>
              <div className="border-t-2 border-gray-800 pt-2 mt-12 mx-8">
                <p className="text-sm font-semibold text-gray-900">Delivered By</p>
        ...
        <div className="flex justify-end space-x-3 no-print">
          ...
          <button
            onClick={() => router.push(`/dashboard/delivery-orders/${deliveryOrder.id}/edit`)}
