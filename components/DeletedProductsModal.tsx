...
import { Product, getDeletedProducts, restoreProduct } from '@/lib/products';
...
export default function DeletedProductsModal({ isOpen, onClose, onRestore }: DeletedProductsModalProps) {
  ...
  const fetchDeletedProducts = async () => {
    ...
    try {
      ...
      if (response.code === 200 && Array.isArray(response.data)) {
        ...
        showError('Failed to fetch deleted products');
      }
    } catch (error) {
      console.error('Error fetching deleted products:', error);
      showError('Error fetching deleted products');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleConfirmRestore = async () => {
    ...
    setRestoring(productToRestore.id);
    setConfirmOpen(false);
    try {
      const response = await restoreProduct(productToRestore.id);
      if (response.code === 200) {
        // Remove from both deleted products and filtered products list
        setDeletedProducts(prev => prev.filter(item => item.id !== productToRestore.id));
        setFilteredProducts(prev => prev.filter(item => item.id !== productToRestore.id));
        showSuccess(`Product "${productToRestore.name}" has been restored successfully!`);
        onRestore?.();
      } else {
        showError(response.message || 'Failed to restore product');
      }
    } catch (error) {
  ...
  return (
    <div className="fixed inset-0 z-[9999] overflow-y-auto" style={{ zIndex: 99999 }}>
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Manage deleted products and restore them if needed
                </p>
              </div>
            </div>
            <button
          ...
          <div className="mb-4 space-y-3">
            ...
            <div className="flex items-center justify-between text-sm text-gray-600">
              <div>
                {searchTerm ? (
                  <span>
                    Showing {filteredProducts.length} of {deletedProducts.length} deleted products
                  </span>
                ) : (
                  <span>{deletedProducts.length} deleted products total</span>
                )}
              </div>
              {searchTerm && (
                <button
          ...
          <div className="max-h-[60vh] overflow-y-auto">
            {loading ? (
              <div className="flex items-center justify-center py-12">
                ...
                <span className="ml-3 text-gray-600">Loading deleted products...</span>
              </div>
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-500">There are no deleted products to display.</p>
              </div>
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-500 mb-4">No deleted products match your search for "{searchTerm}"</p>
                <button
                  onClick={() => setSearchTerm('')}
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-500 mt-1">
                          Deleted on {product.deleted_at ? new Date(product.deleted_at).toLocaleDateString('id-ID', {
                            year: 'numeric',
                            month: 'long',
                            day: 'numeric',
                            hour: '2-digit',
      ...
      <ConfirmModal
        ...
        description={`Are you sure you want to restore product "${productToRestore?.name}"? This will make it available again.`}
        confirmText="Restore"
        cancelText="Cancel"
        type="info"
        isLoading={restoring === productToRestore?.id}
      />
    </div>
  );
}