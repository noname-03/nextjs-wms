...
import { PurchaseOrder, getDeletedPurchaseOrders, restorePurchaseOrder } from '@/lib/purchaseOrders';
import { useAlert } from '@/components/AlertProvider';
import ConfirmModal from '@/components/ConfirmModal';

interface DeletedPurchaseOrdersModalProps {
  isOpen: boolean;
  onClose: () => void;
  onRestore?: () => void;
}

export default function DeletedPurchaseOrdersModal({ isOpen, onClose, onRestore }: DeletedPurchaseOrdersModalProps) {
  ...
  const [orderToRestore, setOrderToRestore] = useState<PurchaseOrder | null>(null);
  const { showSuccess, showError } = useAlert();

  const fetchDeletedOrders = async () => {
    setLoading(true);
    try {
      const response = await getDeletedPurchaseOrders();
      ...
      showError('Error fetching deleted purchase orders');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRestoreClick = (order: PurchaseOrder) => {
  ...
  const handleConfirmRestore = async () => {
    ...
    try {
      const response = await restorePurchaseOrder(orderToRestore.id);
      if (response.code === 200) {
        ...
        showSuccess(`Purchase Order "${orderToRestore.poNumber}" has been restored successfully!`);
        onRestore?.();
      } else {
        showError(response.message || 'Failed to restore purchase order');
      }
    } catch (error) {
      console.error('Error restoring purchase order:', error);
      showError('Error restoring purchase order');
    } finally {
      setRestoring(null);
  ...
  return (
    <div className="fixed inset-0 z-[9999] overflow-y-auto" style={{ zIndex: 99999 }}>
        ...
        >
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h3 className="text-lg font-medium text-gray-900">Deleted Purchase Orders</h3>
                <p className="text-sm text-gray-500">
                  Manage deleted purchase orders and restore them if needed
                </p>
              </div>
            </div>
            <button
          ...
          <div className="mb-4 space-y-3">
            ...
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search deleted purchase orders..."
                value={searchTerm}
            ...
            <div className="flex items-center justify-between text-sm text-gray-600">
              <div>
                {searchTerm ? (
                  <span>
                    Showing {filteredOrders.length} of {deletedOrders.length} deleted purchase orders
                  </span>
                ) : (
                  <span>{deletedOrders.length} deleted purchase orders total</span>
                )}
              </div>
              {searchTerm && (
                <button
          ...
          <div className="max-h-[60vh] overflow-y-auto">
            {loading ? (
              <div className="flex items-center justify-center py-12">
                ...
                <span className="ml-3 text-gray-600">Loading deleted purchase orders...</span>
              </div>
              ...
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-medium text-gray-900 mb-2">No deleted purchase orders</h3>
                <p className="text-gray-500">There are no deleted purchase orders to display.</p>
              </div>
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-500 mb-4">No deleted purchase orders match your search for "{searchTerm}"</p>
                <button
                  onClick={() => setSearchTerm('')}
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center space-x-4 text-sm text-gray-600 mt-1">
                          ...
                          <span>Amount: {formatPrice(order.totalAmount)}</span>
                        </div>
                        <div className="flex items-center space-x-4 text-sm text-gray-600 mt-1">
      ...
      <ConfirmModal
        ...
        description={`Are you sure you want to restore purchase order "${orderToRestore?.poNumber}"? This will make it available again.`}
        confirmText="Restore"
        cancelText="Cancel"
        type="info"
